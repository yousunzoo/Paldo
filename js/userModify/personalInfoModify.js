...
export async function setModifyPage() {
  ...
  function validPasswordCheck(password) {
    ...
    if (password.length < 8 || password.length > 20) {
      ...
      !passwordErrorMesssage.classList.contains("active") && passwordErrorMesssage.classList.add("active");
      newPasswordInputEl.classList.add("error");
      validCheck.newPassword = false;
    } else if (num < 0 || eng < 0) {
  ...
  function validThumbnailCheck(file) {
    ...
    const validFileType = ["image/jpg", "image/jpeg", "image/webp", "image/png", "image/gif", "image/svg+xml"];
