...
export function setOrderListPage() {
  ...
  async function initPage() {
    ...
    const data = pickerEl.value
      ? filterByFlatPickr(orderListData)
      : orderListData;
  ...
  function groupByMinutes(list) {
    return list.reduce((acc, order) => {
      ...
      // 거래 시간으로 그룹핑
      acc[formattedDate]
        ? acc[formattedDate].push(newFormattedProduct)
        : (acc[formattedDate] = [newFormattedProduct]);
      return acc;
    }, {});
  }
    ...
    list.forEach((item) => {
      ...
      const duplicatedOrdersObj = duplicatedOrders.reduce((acc, order) => {
        ...
        acc[productId]
          ...
          : (acc[productId] = { ...order, quantity: 1 });
        return acc;
      }, {});
  ...
  function createOrderListUls(list) {
    return list.map((list, index) => {
      ...
      const orderListUl = makeDOMwithProperties("div", {
      ...
      orderListUl.innerHTML = /* html */ `
        <div class='list-title' id=${index}>
          <div class='title-info'>
            ...
            <span class='transaction-name'>${products[0].title} 포함 총 ${
        products.length
      }건</span>
            <span class='transaction-price'>${price.toLocaleString(
              "ko-KR"
            )}원</span>
          </div>
        ...
        <div class='order-area'>
          <div class='info'>
            ...
            <div class='info-list'>
              ...
              <dl>
                <dt>결제금액</dt>
                <dd id='transactionPrice'>${(
                  product.price * product.quantity
                ).toLocaleString("ko-KR")}원</dd>
              </dl>
            </div>
          </div>
          <div class='side-info'>
            ...
            <div class='buttons' data-id=${product.detailId}>
              <button class='order-cancel-button ${done ? "inactive" : ""}' ${
        done ? "disabled" : ""
      }>주문 취소</button>
              <button class='order-confirm-button ${done}' ${
        done ? "disabled" : ""
      }>거래 확정</button>
            </div>
          </div>
      ...
      if (result.isConfirmed) {
        try {
          ...
          if (res) {
            ...
            // 거래 확정 관련 렌더링
            const orderConfirmBtnEl = liEl.querySelector(
